...
export default function Page({
  ...
  async function chooseFile(file?: File) {
    ...
    if (!r.ok) {
      setMsg(
        "Unable to check existing institutional provenance."
      );
      return;
    }
    ...
    if (
      (d.sameOrganization || []).length
    ) {
      setMsg(
        "This exact file is already registered by this institution. Use versioning or supersession instead of issuing it again."
      );
      return;
    }

    if (
      (d.otherOrganizations || []).length
    ) {
      setDups(d.otherOrganizations);

      setRelatedRecordId(
        d.otherOrganizations[0].id
      );
  ...
  async function uploadEvidence() {
    ...
    setEMsg(
      "Verification evidence uploaded privately."
    );
  ...
  async function issue() {
    ...
    if (!assetHash) {
      return setMsg(
        "Choose the final document file first."
      );
    }
    ...
    <main className="pageShell">
      ...
      <section className="panel">
        <div className="formGrid">
          ...
          <div className="sectionLabel">
            01 · Authorized issuer key
          </div>

          <div className="field full">
            <label>
              Issuer key passphrase
            </label>
          ...
          <div className="field full">
            {pk ? (
              <>
                <label>
                  Registered issuer public key
                </label>

                <code className="publicKey">
                  {pk}
                </code>
              </>
            ) : (
            ...
            <>
              ...
              <div className="field">
                ...
                >
                  ...
                  <option value="co_issuer">
                    Co-issuer
                  </option>

                  <option value="co_signatory">
                    Co-signatory
                  </option>

                  <option value="attestor">
                    Attestor
                  </option>
